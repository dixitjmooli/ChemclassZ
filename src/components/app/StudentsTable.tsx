...
export function StudentsTable() {
  ...
  const [adminDialogOpen, setAdminDialogOpen] = useState(false); // New admin dialog
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      const filteredStudents = students.filter(s => s.school === adminSchool);
      ...
      setDebugInfo(prev => ({
        ...
        rawStudentCount: filteredStudents.length,
        errorMessage: null,
      }));
      toast({
        title: 'Refreshed',
        description: `Found ${filteredStudents.length} students at ${now}`,
      });
    } catch (error) {
  ...
  // Form state - school is auto-filled from admin's school
  ...
  // Admin form state - for creating new institute admin
  ...
  const handleAddStudent = async () => {
    ...
    try {
      await createUser(
        ...
        formData.password,
        adminSchool, // Use admin's school
        'student'
      );
      
      toast({
        title: 'Student Added',
        description: `${formData.name} has been added to ${adminSchool}`,
      });
      
      setFormData({ name: '', username: '', password: '', school: adminSchool });
      setDialogOpen(false);
    } catch (error) {
      toast({
        title: 'Error',
  ...
  // Handle adding new admin for a new institute
  const handleAddAdmin = async () => {
    if (!adminFormData.name || !adminFormData.username || !adminFormData.password || !adminFormData.school) {
      toast({
        title: 'Missing Fields',
        description: 'Please fill in all fields',
        variant: 'destructive',
    ...
    try {
      await createUser(
        adminFormData.name,
        adminFormData.username.toLowerCase(),
      ...
      toast({
        title: 'Admin Created',
        description: `${adminFormData.name} can now login as admin for ${adminFormData.school}`,
      });
      
      setAdminFormData({ name: '', username: '', password: '', school: '' });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Dialog open={adminDialogOpen} onOpenChange={setAdminDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Create New Institute Admin</DialogTitle>
                <DialogDescription>
                  Create an admin account for a new institute/school. They will manage their own students.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4 py-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="admin-school">Institute/School Name</Label>
                  <Input
                    id="admin-school"
                    placeholder="Enter institute name"
                    value={adminFormData.school}
                    onChange={(e) => setAdminFormData({ ...adminFormData, school: e.target.value })}
                  />
                  <p className="text-xs text-muted-foreground">This admin will only see students from this institute</p>
                </div>
              </div>
              <DialogFooter>
                ...
                <Button onClick={handleAddAdmin} disabled={isAdding} className="bg-green-600 hover:bg-green-700">
                  {isAdding ? (
                    <>
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-muted-foreground">Students will be added to your institute</p>
              </div>
            </div>
            <DialogFooter>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold truncate">{adminSchool}</p>
              </div>
            </div>
          </CardContent>
        </Card>
