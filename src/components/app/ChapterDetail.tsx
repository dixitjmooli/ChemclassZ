...
export function ChapterDetail({ onUpdateProgressItem, onUpdateHotsCompleted }: ChapterDetailProps) {
  ...
  // Get chapters from selected subject or store chapters
  ...
  }, [selectedSubjectId, subjects, storeChapters]);
  ...
  // Track previous progress for badge detection
  const prevChapterProgressRef = useRef<number>(0);
  const prevOverallProgressRef = useRef<number>(0);
  ...
  // Reset refs when progress changes
  useEffect(() => {
    const currentProgressId = progress?.id || '';
    if (lastProgressIdRef.current !== currentProgressId) {
      lastProgressIdRef.current = currentProgressId;
      isInitialMountRef.current = true;
      prevChapterProgressRef.current = 0;
      prevOverallProgressRef.current = 0;
    }
  }, [progress?.id]);
  ...
  useEffect(() => {
    if (isInitialMountRef.current) {
      ...
      prevChapterProgressRef.current = chapterProgress;
      prevOverallProgressRef.current = overallProgress;
      return;
    }

    const prevChapterBadge = getChapterBadge(prevChapterProgressRef.current);
    const currentChapterBadge = getChapterBadge(chapterProgress);

    if (currentChapterBadge.threshold > prevChapterBadge.threshold && chapterProgress > prevChapterProgressRef.current && currentChapterBadge.id !== 'locked') {
      triggerBadgeCelebration({
        badgeId: currentChapterBadge.id,
        badgeName: currentChapterBadge.name,
        badgeIcon: currentChapterBadge.icon,
        badgeColor: currentChapterBadge.color,
        badgeBgColor: currentChapterBadge.bgColor,
        type: 'chapter',
    ...
    const prevOverallBadge = getOverallBadge(prevOverallProgressRef.current);
    const currentOverallBadge = getOverallBadge(overallProgress);

    if (currentOverallBadge.threshold > prevOverallBadge.threshold && overallProgress > prevOverallProgressRef.current) {
      triggerBadgeCelebration({
        badgeId: currentOverallBadge.id,
        badgeName: currentOverallBadge.name,
        badgeIcon: currentOverallBadge.icon,
        badgeColor: currentOverallBadge.color,
        badgeBgColor: currentOverallBadge.bgColor,
        type: 'overall',
      });
    }
    ...
    prevChapterProgressRef.current = chapterProgress;
    prevOverallProgressRef.current = overallProgress;
  }, [chapterProgress, overallProgress, chapter?.name, triggerBadgeCelebration]);
  ...
  const handleHotsChange = (checked: boolean) => {
    if (onUpdateHotsCompleted && chapter) {
      onUpdateHotsCompleted(chapter.id, checked);
    }
  };
              ...
              <div className="flex items-center gap-3">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      handleHotsChange(checked as boolean)
                    }
                  />
